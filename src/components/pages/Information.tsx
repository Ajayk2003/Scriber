...
export default function Information() {
  const { output, finished } = useAudioStore()
  const { setTranslation, setTranslating, setTab, tab, translating, toLanguage, translation } =
    useInformationStore()
    ...
    const onMessageReceived = async (e: MessageEvent<WorkerMessage>) => {
      switch (e.data.status) {
        ...
        case 'progress':
          ...
          break
        case 'update':
          setTranslation(e.data.output ? e.data.output : null)
  ...
  }, [setTranslating, setTranslation])

  const textElement =
    tab === 'transcription' ? output?.map((val) => val.text).join(' ') : translation || ''

  function handleCopy() {
    navigator.clipboard.writeText(textElement ? textElement : '')
  }

  function handleDownload() {
    const element = document.createElement('a')
    const file = new Blob([textElement ? textElement : ''], { type: 'text/plain' })
  ...
  return (
    <main className="flex-1 p-4 flex flex-col gap-3 text-center sm:gap-4 justify-center pb-20 max-w-prose w-full mx-auto">
      ...
      <div className="my-8 flex flex-col-reverse max-w-prose w-full mx-auto gap-4">
        ...
        {tab === 'transcription' ? (
          <Transcription textElement={textElement ? textElement : ''} />
        ) : (
          <Translation
